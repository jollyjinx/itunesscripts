...
if( $htmloutput =~ m/<select Id="selReportType" onchange="handleSelReportType\(\)" name="([^"]+)">/io )
{
	$selReportType = $1;
	print "\$selReportType: $selReportType\n";
}
else
{
	print STDERR __LINE__."could not find handleSelDateType action:\n$htmloutput\n";
	exit 1;
}

if( $htmloutput =~ m/<select Id="selDateType" onchange="handleSelDateType\(\)" name="([^"]+)">/io )
{
	$selDateType = $1;
	print "\$selDateType: $selDateType\n";
}
else
{
	print STDERR __LINE__."could not find selDateType action:\n$htmloutput\n";
	exit 1;
}

if( $htmloutput =~ m/<input\s+type="hidden"\s+name="wosid" value="([^"]+)"/io )
{
	$wosid=$1;
	print "\$wosid: $wosid\n";
}
...
{
	print STDERR __LINE__."could not find wosid action:\n$htmloutput\n";
	exit 1;
}
...
{
	my $htmloutput = `$curlcmd -d "$selReportType=Summary" -d "$selDateType=$datetype" -d "hiddenDayOrWeekSelection=$datetype" -d "hiddenSubmitTypeName=ShowDropDown" -d "wosid=$wosid" "$itmsserver$action" ` ;
	...
	{
		print STDERR __LINE__."could not find datetypeaction action:\n$htmloutput\n";
		exit 1;
	}
	...
	while( my($option,$filename) = each(%missingdates) )
	{
		...
		`$curlcmd  -d "$selReportType=Summary" -d "$selDateType=$datetype" -d "$datetypeaction=$option" -d "hiddenDayOrWeekSelection=$datetype" -d "hiddenSubmitTypeName=Download" -d "wosid=wosid" "$itmsserver$subaction" -o $filename`;
	}
}
